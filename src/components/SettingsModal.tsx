...
import { X, Moon, Sun, Globe, EyeOff, Key, Trash2, Save, Sparkles, User, CheckCircle, Camera, Image, FileText, Mic, Shield, Trophy, Volume2, Database, Loader2, Wifi, WifiOff, Sliders, HelpCircle, LogOut, LogIn } from "lucide-react";
    ...
    >
      <div className="relative w-full max-w-2xl bg-white dark:bg-[#11192e] border border-slate-100 dark:border-slate-800 rounded-3xl p-8 shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        {activeTab === "preferences" ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4 p-5 bg-slate-50/50 dark:bg-slate-900/30 border border-slate-100 dark:border-slate-800 rounded-2xl">
                  ...
                  {/* Diagnostics nested inside Support */}
                  <div className="relative overflow-hidden border-t border-slate-150 dark:border-slate-800/60 pt-3.5 space-y-6">
                    {user.isGuest && (
                      <div className="absolute inset-0 bg-white/95 dark:bg-[#11192e]/95 backdrop-blur-[2px] z-10 flex flex-col items-center justify-center p-4 text-center">
                        <Database className="w-8 h-8 text-[#C96A3D] mb-2 animate-bounce" />
                        <p className="text-xs font-bold text-slate-800 dark:text-white">Sign in to use this feature</p>
                        <p className="text-[10px] text-slate-400 mt-1 mb-3">Database sync diagnostics require a registered account.</p>
                        {onOpenAuth && (
                          <button
                            type="button"
                            onClick={() => {
                            ...
                            className="flex items-center gap-1.5 py-1.5 px-3 bg-[#C96A3D] text-white text-[11px] font-bold rounded-xl shadow-3xs hover:bg-[#b0582d] transition-all cursor-pointer active:scale-95"
                          >
                ...
                {/* Account Section */}
                <div className="space-y-4 p-5 bg-slate-50/50 dark:bg-slate-900/30 border border-slate-100 dark:border-slate-800 rounded-2xl relative overflow-hidden">
                  {user.isGuest && (
                    <div className="absolute inset-0 bg-white/95 dark:bg-[#11192e]/95 backdrop-blur-[2px] z-10 flex flex-col items-center justify-center p-4 text-center">
                      <User className="w-8 h-8 text-[#C96A3D] mb-2 animate-bounce" />
                      <p className="text-xs font-bold text-slate-800 dark:text-white">Sign in to use this feature</p>
                      <p className="text-[10px] text-slate-400 mt-1 mb-3">Profile editing requires an active account.</p>
                      {onOpenAuth && (
                        <button
                          type="button"
                          onClick={() => {
          ...
          <div className="relative overflow-hidden min-h-[400px] flex flex-col flex-1">
            {user.isGuest && (
              <div className="absolute inset-0 bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur-[2px] z-10 flex flex-col items-center justify-center p-6 text-center">
                <Trophy className="w-12 h-12 text-[#C96A3D] mb-3 animate-bounce" />
                ...
                <p className="text-xs text-slate-400 mt-1 mb-4 max-w-xs">
                  Achievements tracking, XP gain, and special badges are only available for registered accounts.
                </p>
                {onOpenAuth && (
                  <button
                    onClick={() => {
